...
def loadSpectrometer(filename,multiplyCoordinatesByFactor=1):
	class Holder:
		def __init__(self):
			pass
	s = Holder()
...
#	cfg.get('Parameters','aberrationFreeWavelength')
	...
	s.GratingPoints = s.GratingPoints*multiplyCoordinatesByFactor
	s.GratingWidths = s.GratingWidths*multiplyCoordinatesByFactor
	s.InputPoint = s.InputPoint*multiplyCoordinatesByFactor
	s.InputWidth = s.InputWidth*multiplyCoordinatesByFactor
	s.OutputPoints = s.OutputPoints*multiplyCoordinatesByFactor
	s.OutputWidths = s.OutputWidths*multiplyCoordinatesByFactor
	return s
...
def makeDiffractionGrating(cell,layer,spectrometer):
	...
	for i in range(nGrooves-1):
		...
		points[3] = points[2]-array([10,0])
		points[4] = points[0]-array([10,0])
...
##for path in paths:
##	path.segment(2, 50,path.direction)
