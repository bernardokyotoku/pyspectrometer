...
device.setOutputWavelengths(np.linspace(1550e-9,1600e-9,20))
...
import time
start = time.time()
image = spectrometer.Grid(10e-6,10e-6,1e-7,device.output_list[0].center)
#image.plot()
device.propagateTo(image)
stop = time.time()
image.plotField()
print stop-start
raw_input()
